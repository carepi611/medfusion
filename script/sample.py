...
if __name__ == "__main__":
    ...
    for cond in [0, 1, None]:
        ...
        # ----------- Run --------
        results = pipeline.sample(n_samples, (8, 32, 32), guidance_scale=8, condition=condition, un_cond=un_cond,
                                  steps=steps, use_ddim=use_ddim)
        ...
        utils.save_image(results, path_out / f'test_{cond}.png', nrow=int(math.sqrt(results.shape[0])), normalize=True,
                         scale_each=True)  # For 2D images: [B, C, H, W]
        images[cond] = results

    diff = torch.abs(normalize(rgb2gray(images[1])) - normalize(rgb2gray(images[0])))  # [0,1] -> [0, 1]
    # diff = torch.abs(images[1]-images[0])
    utils.save_image(diff, path_out / 'diff.png', nrow=int(math.sqrt(results.shape[0])), normalize=True,
                     scale_each=True)  # For 2D images: [B, C, H, W]
