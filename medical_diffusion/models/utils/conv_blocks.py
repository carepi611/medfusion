...
class BasicUp(nn.Module):
    def __init__(
        ...
        if learnable_interpolation:
            ...
            # self.calc_shape 函数的目的是根据输入、步长、卷积核大小和填充大小，计算卷积操作后的输出形状
    ...
    def forward(self, x, emb=None):
        if self.learnable_interpolation:
            # 经过calc_shape函数计算后，得到的是卷积操作后的输出形状
            new_size = self.calc_shape(x.shape[2:])
            # 插值操作通过使用已知的数据点来估计未知位置上的数据值。在图像处理中，插值可以用于调整图像的大小、缩放图像、旋转图像等
...
class BasicResBlock(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.basic_block = BasicBlock(spatial_dims, in_channels, out_channels, kernel_size, stride, norm_name, act_name,
                                      dropout, zero_conv)
        Convolution = Conv[Conv.CONV, spatial_dims]
        ...
        # 第一个残差块的输入输出通道数不一致，需要一个1x1的卷积来调整通道数
        # 第二个残差块输入输出通道数一致，不需要调整，直接一个残差连接
...
class UnetBasicBlock(nn.Module):
    ...
    def __init__(
        ...
        self.block_seq = nn.ModuleList([
            BasicBlock(spatial_dims, in_channels if i == 0 else out_channels, out_channels, kernel_size, stride,
                       norm_name, act_name, dropout, i == blocks - 1)
    ...
    def forward(self, x, emb=None):
        # ------------ Embedding ----------
        # 这段代码对存在的嵌入数据进行了处理，通过嵌入层进行变换，并将形状调整为与输入x相匹配的形式。
        # 这样做是为了将嵌入信息与输入数据相结合，并在后续的卷积处理中进行使用。
        if emb is not None:
        ...
        # ----------- Convolution ---------
        # len(self.block_seq) = 2
        n_blocks = len(self.block_seq)
        # enumerate表示枚举，i表示索引，block表示元素
        # 每个 BasicBlock 实例的输入都是上一个 BasicBlock 实例的输出。
        # 然后，当前的 BasicBlock 实例将使用自己的参数和配置来处理输入，并产生输出
...
class UnetResBlock(nn.Module):
    ...
    def __init__(
            ...
            dropout: Optional[Union[Tuple, str, float]] = None,
            emb_channels: int = None,
            blocks=2  # number of residual blocks，相当于ResBlock中有几个残差块
    ):
        super().__init__()
        # nn.ModuleList将多个模块组合成一个模块，从而可以像调用一个模块一样调用多个模块
        self.block_seq = nn.ModuleList([
            # 以列表的形式声明多个残差块
            BasicResBlock(spatial_dims, in_channels if i == 0 else out_channels, out_channels, kernel_size, stride,
                          norm_name, act_name, dropout, i == blocks - 1)
            # 通过for循环声明多个残差块
            for i in range(blocks)
        ])
    ...
    def forward(self, x, emb=None):
        ...
        # ----------- Convolution ---------
        n_blocks = len(self.block_seq)  # len(self.block_seq) = 2
        for i, block in enumerate(self.block_seq):
            x = block(x)
...
class DownBlock(nn.Module):
    def __init__(
        ...
        # ensure_tuple_rep([2,2,2], 2)==(2,2) ensure_tuple_rep(1,2)==(1,1)
        # enable_down=True
        enable_down = ensure_tuple_rep(stride, spatial_dims) != ensure_tuple_rep(1, spatial_dims)
        # down_out_channels=out_channels==[128,256,512]
        ...
        self.down_op = BasicDown(
            ...
            in_channels,
            out_channels,
            kernel_size=downsample_kernel_size,  # downsample_kernel_size = [3, 3, 3]
            stride=stride,
...
class UpBlock(nn.Module):
    def __init__(
        ...
        # ensure_tuple_rep(stride, spatial_dims)==(2,2)
        # ensure_tuple_rep(1, spatial_dims)==(1,1)
        # enable_up=True
