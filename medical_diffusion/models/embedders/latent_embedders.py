...
from medical_diffusion.models.utils.conv_blocks import DownBlock, UpBlock, BasicBlock, BasicResBlock, UnetResBlock, \
    UnetBasicBlock
...
class DiagonalGaussianDistribution(nn.Module):
    ...
    def forward(self, x):
        # torch.chunk()函数将张量按维度dim进行平均切分，返回值为分块后的均值和对数方差张量
        mean, logvar = torch.chunk(x, 2, dim=1)
        # torch.clamp将logvar张量(对数方差)每个元素的夹紧到区间 [-30，20]，并返回结果到一个新张量。
        logvar = torch.clamp(logvar, -30.0, 20.0)
        std = torch.exp(0.5 * logvar)  # 计算标准差std，因为logvar是对数方差，所以要先求指数
        # sample表示从标准正态分布中采样得到的样本
        sample = torch.randn(mean.shape, generator=None, device=x.device)
        # 通过这个转换过程，我们从标准正态分布（均值0 方差1） 转换为具有给定均值(mean)和标准差(std)的对角高斯分布样本
        z = mean + std * sample

        batch_size = x.shape[0]
        var = torch.exp(logvar)  # 计算方差var
        # KL 散度是用于衡量两个概率分布之间差异的指标，KL 散度越大，两个概率分布差异越大
        # 我们的目标是将生成的分布（由均值和方差定义）调整为尽可能接近真实数据的分布
        # 这部分代码首先计算了每个样本的 KL 散度，然后将所有样本的 KL 散度相加并取平均值
...
class VQVAE(BasicModel):
    ...
    def ssim_loss(self, pred, target):
        return 1 - ssim(((pred + 1) / 2).clamp(0, 1), (target.type(pred.dtype) + 1) / 2, data_range=1,
                        size_average=False,
    ...
    def rec_loss(self, pred, pred_vertical, target):
        ...
        loss = 0
        loss += torch.mean(
            self.loss_fct(pred, target) + self.perception_loss(pred, target) + self.ssim_loss(pred, target)) * weights[
                    0]
        ...
        for i, pred_i in enumerate(pred_vertical):
            ...
            loss += torch.mean(
                self.loss_fct(pred_i, target_i) + self.perception_loss(pred_i, target_i) + self.ssim_loss(pred_i,
                                                                                                          target_i)) * \
                    weights[i + 1]
...
class VQGAN(VeryBasicModel):
    def __init__(
        ...
        self.vqvae = VQVAE(in_channels, out_channels, spatial_dims, emb_channels, num_embeddings, hid_chs, kernel_sizes,
                           strides, norm_name, act_name, dropout, use_res_block, deep_supervision,
                           learnable_interpolation, use_attention,
        ...
        self.discriminator = nn.ModuleList([Discriminator(in_channels, spatial_dims, hid_chs, kernel_sizes, strides,
                                                          act_name, norm_name, dropout) for i in
                                            range(len(self.vqvae.outc_ver) + 1)])
    ...
    def _step(self, batch: dict, batch_idx: int, state: str, step: int, optimizer_idx: int):
        ...
        weights = [1 / 2 ** i for i in
                   range(1 + len(pred_vertical))]  # horizontal + vertical (reducing with every step down)
        tot_weight = sum(weights)
        weights = [w / tot_weight for w in weights]
        logging_dict = {}
        ...
        if optimizer_idx == 0:
            ...
            for i, pred_i in enumerate(pred_vertical):
                target_i = F.interpolate(target, size=pred_i.shape[2:], mode=interpolation_mode, align_corners=None)
                img_loss += self.vae_img_loss(pred_i, target_i, self.vqvae.outc_ver[i].conv, step, self.discriminator,
                                              i + 1) * weights[i + 1]
...
class VAE(BasicModel):
    def __init__(
        ...
        self.inc = ConvBlock(
            spatial_dims,  # 2
            in_channels,  # 3
            hid_chs[0],  # [64, 128, 256, 512]
            kernel_size=kernel_sizes[0],  # 3
            stride=strides[0],  # [1, 2, 2, 2]
            act_name=act_name,  # ("Swish", {})
            norm_name=norm_name,  # ("GROUP", {'num_groups': 8, "affine": True})
            emb_channels=None
        )
        ...
        self.encoders = nn.ModuleList([
            DownBlock(
                ...
                in_channels=hid_chs[i - 1],  # in_channels = [64, 128, 256]
                out_channels=hid_chs[i],  # out_channels = [128, 256, 512]
                kernel_size=kernel_sizes[i],  # kernel_size = [3, 3, 3]
                stride=strides[i],  # stride = [2, 2, 2]
                downsample_kernel_size=downsample_kernel_sizes[i],  # downsample_kernel_size = [3, 3, 3]
                norm_name=norm_name,  # norm_name = ("GROUP", {'num_groups': 8, "affine": True})
                act_name=act_name,  # act_name = ("Swish", {})
                dropout=dropout,  # None
                use_res_block=use_res_block,  # True
                learnable_interpolation=learnable_interpolation,  # True
                use_attention=use_attention[i],  # 'none'
                emb_channels=None
            )
            for i in range(1, self.depth)  # self.depth = 4，range(1, 4) = [1, 2, 3]
        ])
        ...
        self.decoders = nn.ModuleList([
            UpBlock(
                ...
                out_channels=hid_chs[i],
                kernel_size=kernel_sizes[i + 1],
                stride=strides[i + 1],  # stride = [2, 2, 2]
            ...
            for i in range(self.depth - 1)  # self.depth-1 = 3
        ])
    ...
    def encode(self, x):
        # h表示每个Resblock的输出
        h = self.inc(x)
        for i in range(len(self.encoders)):  # len(self.encoders) = 3
            h = self.encoders[i](h)  # self.encoders[i](h)表示输入h经过第i个DownBlock的输出
        z = self.out_enc(h)
        z, _ = self.quantizer(z)
        return z
    ...
    def forward(self, x_in):
        ...
        # h得到的是每个Resblock的输出
        h = self.inc(x_in)
        # 如果把for i in range(len(self.encoders))换成for i, encode in enumerate(self.encoders)，那么h = encode(h)
        # 因为self.encoders是一个list，里面有三个DownBlock，所以h = self.encoders[i](h)表示输入h经过第i个DownBlock的输出
        for i in range(len(self.encoders)):  # len(self.encoders) = 3
            h = self.encoders[i](h)
        # 经过两个BasicBlock
        z = self.out_enc(h)
        ...
        # --------- Quantizer 量化 ---------------
        # z_q表示具有给定均值(mean)和标准差(std)的对角高斯分布样本
        # emb_loss表示kl散度
        z_q, emb_loss = self.quantizer(z)
    ...
    def rec_loss(self, pred, pred_vertical, target):
        ...
        for i, pred_i in enumerate(pred_vertical):
            target_i = F.interpolate(target, size=pred_i.shape[2:], mode=interpolation_mode, align_corners=None)
            rec_loss_i = self.loss_fct(pred_i, target_i) + self.perception_loss(pred_i, target_i) + self.ssim_loss(
                pred_i, target_i)
...
class VAEGAN(VeryBasicModel):
    ...
    def vae_img_loss(self, pred, target, dec_out_layer, step, discriminator, depth=0):
        ...
        if (step > self.start_gan_train_step) and (depth < 2):
            gan_loss = -torch.sum(discriminator[depth](
                pred))  # clamp(..., None, 0) => only punish areas that were rated as fake (<0) by discriminator => ensures loss >0 and +- don't cannel out in sum
            lambda_weight = self.compute_lambda(rec_loss, gan_loss, dec_out_layer)
            gan_loss = gan_loss * lambda_weight
